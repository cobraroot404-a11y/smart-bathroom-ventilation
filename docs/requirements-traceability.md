...
| TS-2 | Firmware unit tests (threshold/hysteresis/timers/max-runtime/relay-polarity/startup/invalid-sensor/fallback) | `firmware/test/test_controller/test_main.cpp` | `pio test -e native` (or Dockerized equivalent - see `docs/testing.md`) | 25/25 passing locally, and 25/25 passing via the real `pio test -e native` in CI (`.github/workflows/ci.yml`) - which also caught and led to fixing a missing `test_build_src = true` in `firmware/platformio.ini` that the Docker fallback could not detect | Verified | N/A |
